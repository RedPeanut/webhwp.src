...
export class StorageDirectoryEntry extends DirectoryEntry {
    ...
    eachChild(
        consumer: (directoryEntry: DirectoryEntry) => void,
        stopPredicate?: (dirEntry: DirectoryEntry) => boolean
    ) {
    ...
    find<T extends DirectoryEntry>(name: string): T {
        let result: T;
        ...
        while(true) {
            if(currentNode != null && visitedNodes.indexOf(currentNode.getValue().getId()) === -1) {
                ...
                if(currentNode.getValue().getDirectoryEntryName() === name) {
                    result = currentNode.getValue() as T;
                    break;
                }
